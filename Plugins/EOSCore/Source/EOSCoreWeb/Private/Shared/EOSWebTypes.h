...
public:
	explicit FWebRequestParticipantData() : ClientIp("10.0.0.1"), bHardMuted(false)
	{
	}
...
public:
	...
	FCreateRoomTokenCallback(FString roomName, FString deploymentId, FString clientBaseUrl, FString token, FString puid) : RoomName(roomName), DeploymentId(deploymentId), ClientBaseUrl(clientBaseUrl), Token(token), Puid(puid)
	{
	}
...
public:
	FAccessTokenCallbackData() : ExpiresIn(0)
	{
	}

	FAccessTokenCallbackData(FString accessToken, FString tokenType, FString expiresAt, FString expiresIn, FString nonce, TArray<FString> features, FString organizationId, FString productId, FString sandboxId, FString deploymentId) : AccessToken(accessToken), TokenType(tokenType), ExpiresAt(expiresAt), ExpiresIn(FCString::Atoi(*expiresIn)), Features(features), OrganizationId(organizationId), ProductId(productId), SandboxId(sandboxId), DeploymentId(deploymentId)
	{
	}
...
public:
	FQueryAntiCheatServiceStatusByDeploymentCallbackData() : bServerKick(false)
	{
	}

	FQueryAntiCheatServiceStatusByDeploymentCallbackData(bool bInServerKick) : bServerKick(bInServerKick)
	{
	}
...
public:
	FQueryActiveSanctionsForSpecificPlayerCallbackData() : Timestamp(0), ExpirationTimestamp(0)
	{
	}

	FQueryActiveSanctionsForSpecificPlayerCallbackData(FString InReferenceIdString, int64 InTimestamp, FString InActionString, int64 InExpirationTimestamp) : ReferenceIdString(InReferenceIdString), Timestamp(InTimestamp), ActionString(InActionString), ExpirationTimestamp(InExpirationTimestamp)
	{
	}
...
public:
	FSynchronizeSanctionsToAnExternalServiceCallbackData() : EventType(0), bPending(false), bAutomated(false)
	{
	}
...
public:
	FAuthAccessTokenCallbackData() : ExpiresIn(0)
	{
	}

	FAuthAccessTokenCallbackData(FString accessToken, FString expiresIn, FString expiresAt, FString accountId, FString clientId, FString applicationId, FString tokenId, FString refreshToken, FString refreshExpires, FString refreshExpiresAt) : AccessToken(accessToken), ExpiresIn(FCString::Atoi(*expiresIn)), ExpiresAt(expiresAt), AccountId(accountId), ClientId(clientId), ApplicationId(applicationId), TokenId(tokenId), RefreshToken(refreshToken), RefreshExpires(refreshExpires), RefreshExpiresAt(refreshExpiresAt)
	{
	}
...
public:
	...
	FWebResponse(int32 code, FString message) : Code(code), Message(message)
	{
	}
...
public:
	FSubmitTicketRequest() : Subject(ESubjectType::NOT_SET)
	{
	}
...
struct FInitiateEpicAuthenticationRequest
{
	...
	FInitiateEpicAuthenticationRequest() : RedirectURI("https://eeldev.com/auth/"), Scope("basic_profile friends_list presence"), Timeout(60)
	{
	}
};
...
struct FRequestAuthAccessTokenRequest
{
	...
	FRequestAuthAccessTokenRequest() : GrantType(EWebGrantType::NOT_SET), Scope("basic_profile friends_list presence")
	{
	}
...
struct FSubmitTicketResponse
{
	...
	FSubmitTicketResponse(FString prodName, FString prodSlug, FString guid, FString senderName, FString senderEmail, FString subject, FString message, FString errorCode, FString systemOS, FString systemAntiMalware, FString systemOther, FString timestamp) : ProdName(prodName), ProdSlug(prodSlug), GUID(guid), SenderName(senderName), SenderEmail(senderEmail), Subject(subject), Message(message), ErrorCode(errorCode), SystemOS(systemOS), SystemAntiMalware(systemAntiMalware), SystemOther(systemOther), Timestamp(timestamp)
	{
	}
...
struct FSanctionPostPayload
{
	...
	FSanctionPostPayload() : bPending(false), Duration(0)
	{
	}
};
...
{
	GENERATED_BODY()
	FQuerySanctionAppealsResult() = default;
