...
#else
	  , m_SessionId(FUniqueNetIdString::Create(InSessionId))
#endif
...
#else
	m_SessionId = FUniqueNetIdString::Create(OwnerGuid.ToString());
#endif
}
...
void FOnlineSessionEOSCore::SetAttributes(EOS_HSessionModification SessionModHandle, FNamedOnlineSession* Session)
{
	...
	FString CustomServerPort;
	FParse::Value(FCommandLine::Get(), TEXT("Port="), CustomServerPort);

	if (CustomServerPort.Len() > 0)
	{
		FAttributeOptions Opt9("ServerPort", CustomServerPort);
...
uint32 FOnlineSessionEOSCore::StartEOSSession(FNamedOnlineSession* Session)
{
	...
	//{
	//	EOS_AntiCheatServer_AddNotifyMessageToClientOptions AddNotifyMessageToClientOptions;
	//	AddNotifyMessageToClientOptions.ApiVersion = EOS_ANTICHEATSERVER_ADDNOTIFYMESSAGETOCLIENT_API_LATEST;
	//	
	//	EOS_AntiCheatServer_AddNotifyMessageToClient(m_Subsystem->m_AntiCheatServerHandle, &AddNotifyMessageToClientOptions, nullptr, [](const EOS_AntiCheatCommon_OnMessageToClientCallbackInfo* Data)
	//	{
	//		UE_LOG_ONLINE_SESSION(Log, TEXT("OnMessageToClientCallbackInfo: %s"), UTF8_TO_TCHAR(Data->MessageData));
	//	});
	//}
	//
	//{
	//	EOS_AntiCheatServer_AddNotifyClientActionRequiredOptions AddNotifyClientActionRequiredOptions;
	//	AddNotifyClientActionRequiredOptions.ApiVersion = EOS_ANTICHEATSERVER_ADDNOTIFYCLIENTACTIONREQUIRED_API_LATEST;
	//	
	//	EOS_AntiCheatServer_AddNotifyClientActionRequired(m_Subsystem->m_AntiCheatServerHandle, &AddNotifyClientActionRequiredOptions, nullptr, [](const EOS_AntiCheatCommon_OnClientActionRequiredCallbackInfo* Data)
	//	{
	//		UE_LOG_ONLINE_SESSION(Log, TEXT("OnClientActionRequiredCallbackInfo: %s"), UTF8_TO_TCHAR(Data->ActionReasonDetailsString));
	//	});
	//}
	//
	//{
	...
	//	BeginSessionOptions.RegisterTimeoutSeconds = 60;
	//	BeginSessionOptions.ServerName = new char[256];
	//	const FTCHARToUTF8 ConvertedServerName(Session->SessionName.ToString());
	//	BeginSessionOptions.ServerName = ConvertedServerName.Get();
	//
	//	const EOS_ProductUserId UserId = EOS_ProductUserId_FromString("00026cf7c36e4a508e4f163003b31b28");
	//	BeginSessionOptions.LocalUserId = UserId;
	//									
	//	EOS_AntiCheatServer_BeginSession(m_Subsystem->m_AntiCheatServerHandle, &BeginSessionOptions);
	//}
...
bool FOnlineSessionEOSCore::RegisterPlayers(FName SessionName, const TArray<TSharedRef<const FUniqueNetId>>& Players, bool bWasInvited)
{
	...
	EOS_Sessions_RegisterPlayersOptions RegisterPlayersOptions = {
		EOS_SESSIONS_REGISTERPLAYERS_API_LATEST, TCHAR_TO_UTF8(*SessionName.ToString()), PlayersToRegister.GetData(), static_cast<uint32_t>(PlayersToRegister.Num())
	};
...
bool FOnlineSessionEOSCore::UnregisterPlayers(FName SessionName, const TArray<TSharedRef<const FUniqueNetId>>& Players)
{
	...
	EOS_Sessions_UnregisterPlayers(m_Subsystem->m_SessionsHandle, &RegisterPlayersOptions, nullptr, m_Callback);
