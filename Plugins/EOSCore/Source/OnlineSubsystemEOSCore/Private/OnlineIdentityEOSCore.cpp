...
void FOnlineIdentityEOSCore::ConnectLoginStatusChanged(const EOS_Connect_LoginStatusChangedCallbackInfo* data)
{
	...
	if (data->CurrentStatus == EOS_ELoginStatus::EOS_LS_NotLoggedIn)
	{
		if (ProductUserIdToUserNumMap.Contains(data->LocalUserId))
		{
			...
			FUniqueNetIdEOSCorePtr UserNetId = UserNumToNetIdMap[LocalUserNum];
...
bool FOnlineIdentityEOSCore::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	...
	else if (LoginCredentialsType == TEXT("9"))
	{
		...
		OpenIDLoginOptions.UserLoginInfo = NULL;
	...
	else if (LoginCredentialsType == TEXT("10"))
	{
		...
		CallbackObj->m_CallbackLambda = [LocalUserNum, AccountCredentials, this](const EOS_Connect_LoginCallbackInfo* Data)
		{
			if (Data->ResultCode == EOS_EResult::EOS_Success)
			{
				...
				// these are no longer needed since the "Logged in status" callback will trigger these delegates
...
void FOnlineIdentityEOSCore::ConnectLogin(int32 LocalUserNum)
{
	LogVerbose("");
...
void FOnlineIdentityEOSCore::RefreshConnectLogin(int32 LocalUserNum)
{
	...
	LogVerbose("ConnectLogin Status: %s", *LoginStatusToString(AuthLogin));
	...
	else
	{
		...
		CopyProductUserInfoOptions.TargetUserId = ProductUserId;

		EOS_Connect_ExternalAccountInfo* ExternalAccountInfo = NULL;
		if (EOS_Connect_CopyProductUserInfo(m_Subsystem->m_ConnectHandle, &CopyProductUserInfoOptions, &ExternalAccountInfo) == EOS_EResult::EOS_Success)
		{
			LogVerbose("Fetched external account info");
		}

		// try logging in using device id
		...
		{
			DeviceIdUserLoginInfo.DisplayName = ExternalAccountInfo->DisplayName;
		}
		else
		{
...
void FOnlineIdentityEOSCore::AddLocalUser(int32 LocalUserNum, EOS_EpicAccountId EpicAccountId, EOS_ProductUserId UserId)
{
	...
	FUniqueNetIdEOSCoreRef UserNetId(new FUniqueNetIdEOSCore(NetId));
